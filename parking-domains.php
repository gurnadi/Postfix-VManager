...
if(ereg("parking-domains.php", $_SERVER['PHP_SELF']) || decoded($_SESSION['role']) != 'SuperAdmin'){
  header ("Location: /");
  exit;
}
  ...
  if(isset($_GET['domain']))
  {
    ...
    $get_domains = $db->get_count($TBL_backupmx,'*',"domain = :ldomain", $bind);
  ...
  elseif(isset($_GET['srch']))
  {
    ...
    $get_domains = $db->get_count($TBL_backupmx,'*',"domain LIKE :ldomain", $bind);
    $total_domains = $get_domains[0]['count(*)'];
  }  
  else
  {
    $get_domains = $db->get_count($TBL_backupmx,'*');
    $total_domains = $get_domains[0]['count(*)'];
  }
    ...
    $self_url = (isset($url) && $url != '' && strpos($url,'page=') !== true) ?  "/".$url : '/parking-domains';
    ...
    if(isset($_POST['limit']))
      $_SESSION['pdSession'] = $_POST['limit'];

    $maxRows = isset($_SESSION['pdSession']) ? $_SESSION['pdSession'] : $CONF['page_size'];
    $pageNum = 1;
  ...
  if(isset($_GET['domain']))
    $domains = $db->select($TBL_backupmx,"domain = :ldomain", $bind);
  elseif(isset($_GET['srch']))
    $domains = $db->select($TBL_backupmx,"domain LIKE :ldomain", $bind,"ORDER BY $order LIMIT $startRow, $maxRows");
  else
    $domains = $db->select($TBL_backupmx,'','',"ORDER BY $order LIMIT $startRow, $maxRows");
?>
...
<?php 
  if(!isset($_GET['domain']))
  echo '<div style="float: left; margin-top: 5px;" id="totalDomains"><strong>Total Number of Parking Domains : '.$total_domains.'</strong></div>';
?>
...
<td class="event-label" style="border-right: 0; background-color:#FFFFFF; font-weight: bold; width: 280px; color: #004A95;"><center>SMTP Relay Host</center></td>
<td class="<?php if($order == 'modified ASC') echo 'modSortasc'; else echo 'modSortdesc';?>" style="background-color: rgba(141, 192, 219, 0.25); text-shadow: 0 1px 1px rgba(255, 255, 255, 0.75); width: 112px;"><center>Modified</center></td>
...
foreach ($domains as $domain)
{
  ...
  echo '<td style="padding-top: 9px;" class="domain-label">'.$domain['domain'].'</td>';
  echo '<td style="padding-top: 9px;"><center>'.$domain['smtp_host'].'</center></td>';
...
<li><a href="/edit-parking-domain?domain='.encoded($domain['domain']).'"><i class="icon-edit"></i> Edit Domain</a></li>
<li class="divider"></li> 
...
<?php 
 ...
 if(isset($_GET['srch']))
 {
   ...
   $reqUrl = '/parking-domains?srch='.$reqUri[0];
 }
 else
 {
  $reqUrl = '/parking-domains';
 }

/* create oder url */
 $odUrl = (strpos($_SERVER['REQUEST_URI'],'page=') !== false) ? '/parking-domains' : $reqUrl;
 if(preg_match('/orderby=ASC|orderby=DESC|orderby=modASC|orderby=modDESC/', $reqUrl))
 {
   $odUri = explode('?',$reqUrl);
     $odUrl = '/parking-domains?'.$odUri[1];
 }
...
$('.delete').click( function (){
	...
	var page = '<?php echo $pageNum;?>';
	  ...
	  $('#img_load').show();
            ...
            $.post("/remove_parking_domain.php",{'domainID':id},function(response){
...
$('.srgo').click(function (){
	...
	{
	  window.location = "/parking-domains?srch="+site+"<?php echo $srQry;?>";
	  return false;
	}
    });
